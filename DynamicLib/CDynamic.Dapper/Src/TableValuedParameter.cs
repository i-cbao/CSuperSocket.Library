...
namespace Dapper
{
    ...
    internal sealed class TableValuedParameter : SqlMapper.ICustomQueryParameter
    {
        ...
        private static readonly Action<Npgsql.NpgsqlParameter, string> setTypeName;
        static TableValuedParameter()
        {
            var prop = typeof(Npgsql.NpgsqlParameter).GetProperty("TypeName", BindingFlags.Instance | BindingFlags.Public);
            if (prop != null && prop.PropertyType == typeof(string) && prop.CanWrite)
            {
                setTypeName = (Action<Npgsql.NpgsqlParameter, string>)
                    Delegate.CreateDelegate(typeof(Action<Npgsql.NpgsqlParameter, string>), prop.GetSetMethod());
            }
        }
...
#pragma warning restore 0618
            ...
            if (!string.IsNullOrEmpty(typeName) && (parameter is Npgsql.NpgsqlParameter sqlParam))
            {
                setTypeName?.Invoke(sqlParam, typeName);
                
                sqlParam.DbType = DbType.String;//SqlDbType.Structured;
            }
        }
    }
}
