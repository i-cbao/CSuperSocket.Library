...
namespace CSuperSocket.SocketEngine
{
    ...
    public static class BootstrapFactory
    {
        ...
        {
#if !NETSTANDARD2_0
            var configSection = ConfigurationManager.GetSection("CSuperSocket");

            if (configSection == null)//to keep compatible with old version
                configSection = ConfigurationManager.GetSection("socketServer");

            if (configSection == null)
                throw new ConfigurationErrorsException("Missing 'CSuperSocket' or 'socketServer' configuration section.");

            var configSource = configSection as IConfigurationSource;
            if (configSource == null)
                throw new ConfigurationErrorsException("Invalid 'CSuperSocket' or 'socketServer' configuration section.");
        ...
        public static IBootstrap CreateBootstrapFromConfigFile(string configFile, string configSectionName = "")
        {
            ...
            var configSection = config.GetSection("CSuperSocket");
